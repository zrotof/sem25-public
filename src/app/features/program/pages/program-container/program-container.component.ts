...
export class ProgramContainerComponent implements OnInit {
  ...
  onGetProgranByIdEventHandler(event: any) {
    const program = this.programService.getProgramById(event);

    this.ref = this.dialogService.open(ProgramModalComponent, {
      data: {
        program: program
      },
      width: '100%',
      height: '100%',
      baseZIndex: 99999,
      styleClass: "program-modal-style-class"
    });
  }
}
